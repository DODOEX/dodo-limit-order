...
import {ISETTLEXApproveProxy} from "./intf/ISETTLEXApproveProxy.sol";
import {IERC1271Wallet} from "./intf/IERC1271Wallet.sol";
import {InitializableOwnable} from "./lib/InitializableOwnable.sol";
import "./lib/ArgumentsDecoder.sol";

/**
 * @title SETTLEXLimitOrder
 * @author SETTLEX team
 */
contract SETTLEXLimitOrder is EIP712("SETTLEX Limit Order Protocol", "1"), InitializableOwnable, ReentrancyGuard {
    ...
    address public _SETTLEX_APPROVE_PROXY_;
    ...
    function init(address owner, address settlexApproveProxy, address feeReciver) external {
        initOwner(owner);
        _SETTLEX_APPROVE_PROXY_ = settlexApproveProxy;
    ...
    function fillLimitOrder(
        ...
        require(order.taker == msg.sender, "SLOP:PRIVATE_ORDER");
        ...
        //Maker => Taker
        ISETTLEXApproveProxy(_SETTLEX_APPROVE_PROXY_).claimTokens(order.makerToken, order.maker, msg.sender, curMakerFillAmount);
        ...
        if(takerInteraction.length > 0) {
            ...
            require(isWhiteListed[msg.sender], "SLOP: Not Whitelist Contract");
